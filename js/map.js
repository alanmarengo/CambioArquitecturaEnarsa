function ol_map() {
	...
	this.map.create = function() {
		this.baselayers.openstreets = new ol.layer.Tile({
			...
			source: new ol.source.XYZ({
				...
				//	  //{a-c}.tile.openstreetmaps.org/{z}/{x}/{y}.png
		...
		this.baselayers.bingmaps = new ol.source.BingMaps({
			...
			//At82OQs2nduy8Fs8zK6mA0pIEoSUej4XjAXEygroKk062HaA8sBbvAZ0rlZvBp_r 
			//AmqIEhx8ko1O3p1Npagu9_Egw7e8quBgM03p6_xdFqjSfJa6kWv_iUU2nO1htz1G -> 1ra
			imagerySet: 'Road',
			culture: 'ar-ES',
			visible:false,
			crossOrigin: 'anonymous'
				...
				this.getLayers().forEach(function (layer, i, layers) {		
					...
					if ((layer.getVisible()) && (isBase == false)) {
						...
						if(layer.getSource().getGetFeatureInfoUrl) {						
							...
							let urlPrepare = url
							let urlSplitA = urlPrepare.split('/geoserver/')
							let urlSplitB = urlSplitA[1].split('/ows?')
							let espacioTrabajoUrl = urlSplitB[0]
							...
							if (req.responseText != "") {
								...
								if (req.responseText != "") {
									...
									if (html.length > 0) {
										...
										map.preparseGFI(html,"popup-info","info-wrapper", espacioTrabajoUrl); // PARA ACOMODAR RESPUESTA A ESTRUCTURA DE IEASA
								...
								urlJson = layer.getSource().getGetFeatureInfoUrl(evt.coordinate, viewResolution, 'EPSG:3857', {
								...
								if(reqJson.status === 200){
									if(reqJson.responseJSON.features.length > 0){

										const getName = document.querySelectorAll('.layer-group')
										let nombreCapa = ''						
										for(let i=0; getName.length > i; i++){									
											if(getName[i].dataset.layerName === layer.values_.name){
												nombreCapa = getName[i].childNodes[1].childNodes[5].textContent
											}
										}

										let layerGFI = ''
										for(let a=0; reqJson.responseJSON.features.length > a; a++){
											let entries = Object.entries(reqJson.responseJSON.features[a].properties);
											for(let o=0; entries.length > o; o++){
												layerGFI += `<div class="gfiInfo"> <div class="gfiKey">${entries[o][0]} </div> <div class="gfiValue">${entries[o][1]}</div></div> `
											}
											layerGFI += '<hr/>	'
										}
										...
										let htmlJson = `
											<div id="infoGFI"> 
												<div class="titleGFI" onclick=this.parentElement.classList.toggle("activeGFI")>

													<div class="titleIcon" id="iconPlus"> 	 	
														<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256zM256 368C269.3 368 280 357.3 280 344V280H344C357.3 280 368 269.3 368 256C368 242.7 357.3 232 344 232H280V168C280 154.7 269.3 144 256 144C242.7 144 232 154.7 232 168V232H168C154.7 232 144 242.7 144 256C144 269.3 154.7 280 168 280H232V344C232 357.3 242.7 368 256 368z"/>
														</svg>
													</div>
													<div class="titleIcon" id="iconMinus"> 		
														<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256zM168 232C154.7 232 144 242.7 144 256C144 269.3 154.7 280 168 280H344C357.3 280 368 269.3 368 256C368 242.7 357.3 232 344 232H168z"/>
														</svg>
													</div>

													<div class="titleText">${nombreCapa} </div>
												</div>
												<div class="gfiContent">
													${layerGFI}													
												</div>																						
											</div>
										`
										...
										document.getElementById('info-wrapper').innerHTML += htmlJson
										jwindow.open('popup-info');
									}

								}else{
									console.log('Status de la peticion diferente a 200')
								}
	...
	this.map.preparseGFI = function(response,containerID,wrapperID,espacioTrabajoUrl) {
		...
		this.parseGFI(newnodes,containerID,wrapperID,espacioTrabajoUrl);
	
	}
	
	this.map.parseGFI = function(response,containerID,wrapperID,espacioTrabajoUrl) {
		...
		$("#popup-results").children().each(function(i,v) {
			...
			var gid = $(v).attr("x");
			var layer_name = espacioTrabajoUrl+":"+$(v).attr("y");
	...
	this.panel.PreviewLayer = function(layer_id) {
		...
		if (!document.getElementById("layer-checkbox-"+layer_id).layer_preview) {
			...
			let baseURL = layer_wms;

			if(layer_wms.includes('/geoserver/')){			
				if(layer_name.includes(':')){
					let layerNameSplit = layer_name.split(':')
					espacioTrabajo = layerNameSplit[0]+'/';
				}
				let urlSplit = layer_wms.split('/geoserver/')
				baseURL = urlSplit[0]+'/geoserver/'+espacioTrabajo+urlSplit[1]
			}
...
/* 		console.log('#### create layer ####')
		...
		var layerv2 = document.getElementById("layer-checkbox-"+layer_id).layer;
